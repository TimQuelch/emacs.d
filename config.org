...
#+begin_src emacs-lisp
  (use-package gcmh
    :hook (after-init . my/enabled-gcmh-deferred)
    :preface
    (defun my/enabled-gcmh-deferred ()
...
#+begin_src emacs-lisp
  ...
  (defun my/skip-all-but-available-unscheduled-todos ()
    ...
    (save-restriction
      (widen)
      (unless (and (or (my/is-standalone-task-p)
                       (string-equal (org-get-todo-state) my/next-todo-keyword))
