...
** Fonts
Define the fonts that I want to use
#+begin_src emacs-lisp
  (defvar tq/font (font-spec :family "Iosevka" :size 18))
  (defvar tq/variable-width-font (font-spec :family "DejaVu Sans"))
#+end_src

Define function to initialise the main font
#+begin_src emacs-lisp
  (defun tq/init-main-font ()
    "Load the main monospace font"
    (cond (tq/font (cl-pushnew (cons 'font (font-xlfd-name tq/font))
                               default-frame-alist :key #'car :test #'eq))))

  (add-hook 'after-init-hook 'tq/init-main-font)
#+end_src

The below function enables both the fixed-pitch and variable pitch font faces. This should be called after the theme is loaded (?)
#+begin_src emacs-lisp
  (defun tq/init-other-fonts (&optional frame)
    "Set faces for other fonts"
    (with-selected-frame (or frame (selected-frame))
      (set-face-attribute 'fixed-pitch nil :font tq/font)
      (set-face-attribute 'variable-pitch nil :font tq/variable-width-font)))
#+end_src
...
I'm using =straight= as my package manager. I prefer it because it allows me to be more up to date with the upstream repos when I'm tinkering with packages. I also really like how the config file is the authority on what pacakges are enabled/installed unlike =package.el=

First we bootstrap =straight=
#+begin_src emacs-lisp
  (let ((bootstrap-file
  ...
  (setq straight-vc-git-default-clone-depth 'full) ; Clone full repos
#+end_src

I'm using =use-package=  to manage all my modules. It itself must be bootstrapped directly with straight.
#+begin_src emacs-lisp
...
Echo keystrokes in the echo area (faster than default)
#+begin_src emacs-lisp
(setq echo-keystrokes 0.02)
#+end_src

Only prompt for y/n rather than full yes/no
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package hl-line
    ...
    :config
    ;; Don't render hl-line in multiple buffers (more performance)
...
Setting the org directory based on the user's home directory (this might not be portable?). This is in the init section of the use-package form so it is available for use by other configuration components.
#+begin_src emacs-lisp :noweb-ref org-init :tangle no
...
* Meta
I want to automatically tangle and compile this config file on save. ~tq/tangle-and-compile-config~ is provided in =init.el= and is added as a ~after-save-hook~ in the file local variables of this org file.

Finally, we provide ~config~ so that the require in =init.el= can find this.
#+begin_src emacs-lisp
  (provide 'config)
#+end_src
...
# Local Variables:
# eval: (add-hook 'after-save-hook 'tq/tangle-and-compile-config nil t)
# End:
