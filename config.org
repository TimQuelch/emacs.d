...
Setup the default backends. By default doom includes ~company-dabbrev~ and ~company-yasnippet~. Dabbrev adds so much noise, I don't use yasnippet.
...
Also set up completion to trigger everywhere, not just on link start.
#+begin_src emacs-lisp :tangle no :noweb-ref org-roam-config
...
(set-company-backend!
  ...
  '(notmuch-company :with company-ispell))
#+end_src
