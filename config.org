...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    :bind (
           ...
           ("C-c n c" . org-roam-graph-show-connected-component)
           :map org-mode-map
...
#+begin_src emacs-lisp
  (defun my/org-roam-open-ref-note (key)
    (let ((match (assoc key (org-roam-sql [:select [ref file] :from refs]))))
      (if match
          ;; notes already exists, open those notes
          (find-file (cadr match))
        ;; notes do not exist, create new notes
        (let* ((bibtex-completion-bibliography (org-ref-find-bibliography))
               (entry (bibtex-completion-get-entry key))
               (get-and-strip (lambda (field)
                                (let ((value (bibtex-completion-get-value field entry)))
                                  (if value (replace-regexp-in-string
                                             "[{}]" ""
                                             (s-collapse-whitespace value))))))
               (path (f-join org-roam-directory
                             (concat (format-time-string "%Y%m%d%H%M%S" (current-time))
                                     "-" key ".org"))))
          (find-file path)
          (insert
           (concat "#+TITLE: "
                   (funcall get-and-strip "title")
                   "\n#+ROAM_KEY: " key "\n\n"))))))
  (setq org-ref-notes-function 'my/org-roam-open-ref-note)
#+end_src

#+begin_src emacs-lisp
  (helm-delete-action-from-source "Edit notes" helm-source-bibtex)
  (helm-add-action-to-source "Edit notes"
                             'my/org-roam-open-ref-note helm-source-bibtex 7)
#+end_src
...
#+begin_src emacs-lisp
  (setq notmuch-fcc-dirs '(("tim@tquelch.com" . "\"personal/Sent Items\" -unread -inbox +sent")
                           ("tim@quelch.name" . "\"oldpersonal/[Gmail].All Mail\" -unread -inbox +sent")
                           ("t.quelch@qut.edu.au" . "\"qut/Sent Items\" -unread -inbox +sent")
                           ("timothy.quelch@connect.qut.edu.au" . "\"student/Sent Items\" -unread -inbox +sent")))
#+end_src

** TODO configure FCC dirs
